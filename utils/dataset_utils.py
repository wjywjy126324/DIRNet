...
class PromptTrainDataset(Dataset):
    ...
    def _init_clean_ids(self):
        ref_file = self.args.data_file_dir + "\\noisy\\denoise.txt"
        temp_ids = []
    ...
    def _init_hazy_ids(self):
        ...
        self.hazy_ids = self.hazy_ids * 13
...
class DenoiseTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clean_img = crop_img(np.array(Image.open(clean_id).convert('RGB')), base=16)
        clean_name = os.path.splitext(os.path.basename(clean_id))[0]
...
class DerainDehazeDataset(Dataset):
    ...
    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            ...
            name = os.path.basename(degraded_name).split('_')[0] + '.jpg'
            gt_name = os.path.join(dir_name, name)
        return gt_name
